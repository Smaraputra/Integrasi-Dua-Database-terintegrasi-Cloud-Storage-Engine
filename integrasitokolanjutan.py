                            ...
                            for jsonData in json_dict['tb_integrasi_invoice']:
                                if (jsonData['operation'] !='delete'):
                                    ...
                                    if (jsonData['operation'] == 'insert'):
                                        ...
                                        insert(histories_1[0],val,curToko,connToko)
                                        insert(tables_1[0],val,curToko,connToko)
                                    ...
                                    if (jsonData['operation'] =='update'):
                                        ...
                                        update(histories_1[0],val,curToko,connToko)
                                        update(tables_1[0],val,curToko,connToko)
                                else:
                                    data = []
                                    data.append(jsonData['id_invoice'])
                                    val = (data[0])
                                    ...
                                    delete(histories_1[0],val,curToko,connToko)
                                    delete(tables_1[0],val,curToko,connToko)
        ...
        if (len(result) < len(integrasi)):
            ...
            for dataIntegrasi in integrasi:
                ...
                if (a == 0):
                    ...
                    delete(histories_1[0],dataIntegrasi[0],curToko,connToko)
                    if (connection_to_bank == 1):
                        timestr = time.strftime("%Y%m%d-%H%M%S")
                        backupfile = 'bank_' + timestr + '.json'
